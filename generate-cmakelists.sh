...
generate_executable()
{
  ...
  # Loop through .c files for the key and echo their paths
  for c_file in ${c_files[$key]}; do
    echo "    \${CMAKE_SOURCE_DIR}/${c_file}"
  done
  ...
  # Loop through .h files for the key and echo their paths
  for h_file in ${h_files[$key]}; do
    echo "    \${CMAKE_SOURCE_DIR}/${h_file}"
  done
...
generate_commands() {
  ...
  # Create variables for all .c and .h files
  local all_c_files=""
  local all_h_files=""
  local last_key=""

  # Loop through the associative arrays and combine files
  for key in "${!c_array[@]}"; do
    all_c_files+=" \${${key}_SOURCES}"
    all_h_files+=" \${${key}_HEADERS}"
    last_key="$key"
  done
  ...
  echo "    COMMAND \${CLANG_FORMAT} --style=file -i ${all_c_files} ${all_h_files}"
  ...
  echo "add_custom_command("
  echo "    TARGET $last_key POST_BUILD"
  echo "    COMMAND \${CPPCHECK} --error-exitcode=1 --force --quiet --library=posix --enable=all --suppress=missingIncludeSystem --suppress=unusedFunction --suppress=unmatchedSuppression ${all_c_files} ${all_h_files}"
  echo "    WORKING_DIRECTORY \${CMAKE_SOURCE_DIR}"
...
main()
{
    ...
    while read -r line; do
      ...
      for ((i=1; i<${#words[@]}; i++)); do
        # Determine if it's a .c or .h file
        file=${words[i]}
        ext="${file##*.}"
    ...
    generate_commands c_files h_files >> "$output_file"
    echo "" >> "$output_file"
